...
namespace Util {
    ...
    void DrawDebugQuad(const Util::Vec2F middle, const float halfWidth, const float rotation, const Util::Vec3F color) {
        ...
        Util::Vec2F p2 = (middle+Util::Vec2F(halfWidth, halfWidth)).rotate(rotation+0.5f*(float)PI);
        Util::Vec2F p3 = (middle+Util::Vec2F(halfWidth, halfWidth)).rotate(rotation+1.0f*(float)PI);
        Util::Vec2F p4 = (middle+Util::Vec2F(halfWidth, halfWidth)).rotate(rotation+1.5f*(float)PI);
        target->DebugLine(p1, p2, color);
        target->DebugLine(p2, p3, color);
        target->DebugLine(p3, p4, color);
        target->DebugLine(p4, p1, color);
