...
namespace Renderer {
    ...
    public:
        ...
        virtual void RenderTexture(Utils::AreaU8* texture, const Utils::AreaU32 area, const size_t id) = 0;
    ...
    public:
        ...
        void Init();
        void Cleanup(Vulkan::Context& context, Vulkan::Pipeline& boundToPipeline);

        void StartLoading();
        uint32_t AddTextureLoader(std::unique_ptr<AssetLoader> textureLoader);
        void SetCacheName(const std::string name);
        void EndLoading(Vulkan::Context& context, Vulkan::Pipeline& bindToPipeline);

        uint8_t* GetRenderInfo(const uint32_t id);

    private:
    
        std::vector<std::unique_ptr<AssetLoader>> _assetLoaders;
        std::vector<std::vector<uint8_t>> _renderInfos;
